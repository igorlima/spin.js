describe('Usage Option Test', function() {
  ...
  describe('Spin Test Page', function() {
    ...
    it('should have 34 div elements, when create a spinner as {lines: 17}', function() {
      runSpinner( {lines: 17} );
      result = $(target).find('.spinner div').length;
      assert( result === 34, "The number of lines should be 34. The double of 17." );
    });

    it('should have 12px of width, when create a spinner as {length: 7}', function() {
      runSpinner( {length: 7} );
    ...
    it('should have 8px of height, when create a spinner as {width: 8}', function() {
      runSpinner( {width: 8} );
      result = $(target).find('.spinner div div').first().css('height');
      assert( result === "8px", "The height of spinner should be 8px." );
    });

    it('should have 21px of translate, when create a spinner as {radius: 21}', function() {
    ...
    it('should have 25px of border radius, when create a spinner as {corners: 10}', function() {
    ...
    it('should have 6deg of rotate, when create a spinner as {rotate: 6}', function() {
      runSpinner( {rotate: 6} );
      result = $(target).find('.spinner div div').first().css('transform');
      assert( result === "rotate(6deg) translate(10px, 0px)", "The rotate of spinner should be 6deg." );
    });

    it('should clockwise, when create a spinner as {direction: 1}', function() {
    ...
    it('should counterclockwise, when create a spinner as {direction: -1}', function() {
      runSpinner( {direction: -1} );
      result = $(target).find('.spinner > div').first().attr('style');
    ...
    it("should be red, when create a spinner as {color: '#f00'}", function() {
    ...
    it('should be faster (4 laps per second), when create a spinner as {speed: 4}', function() {
      runSpinner( {speed: 4} );
      result = $(target).find('.spinner > div').first().attr('style');
      assert( result.indexOf('0.25s') !== -1, "The spinner should have a speed as 4 laps per second." );
    });

    it('should be trails as 50, when create a spinner as {trail: 50}', function() {
      runSpinner( {trail: 50} );
      result = $(target).find('.spinner > div').first().attr('style');
      assert( result.indexOf('opacity-50-25-0-12') !== -1, "The spinner should have a trail as 50." );
    });

    it('should have shadow, when create a spinner as {shadow: true}', function() {
    ...
    it("should have 'indicator' as class name, when create a spinner as {className: 'indicator'}", function() {
